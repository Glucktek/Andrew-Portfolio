...
export async function updateAstroConfig(localeConfig, logOptions) {
  ...
  let astroConfig;
  try {
    astroConfig = await fsPromises.readFile(
      ACTIVE_PATHS.astroConfigPath,
      "utf8",
    );
  } catch (error) {
  ...
  const currentLocales = currLocales
    .split(",")
  ...
  if (logOptions.enabled) {
    log(
      `Locale operations determined: 
		...
		- Edit old default to new default: ${updatedLocaleConfig.editOldDefaultToNewDefault}`,
      logOptions,
    );
  }
  // Create new locales string for the Astro config
  const newLocalesString = updatedLocaleConfig.newLocales
    .map((locale) => `"${locale}"`)
    .join(", ");
  // Update the Astro config with new locale settings
  astroConfig = astroConfig.replace(
    `defaultLocale: "${defaultLocale}"`,
    `defaultLocale: "${updatedLocaleConfig.newDefaultLocale}"`,
  );
  astroConfig = astroConfig.replace(
    `locales: [${currLocales}]`,
...
function determineLocaleOperations(localeConfig) {
  const { defaultLocale, newDefaultLocale, currentLocales, newLocales } =
    localeConfig;
  ...
  if (currentLocales.includes(newDefaultLocale)) {
    ...
    localeConfig.localesToAdd = newLocales.filter(
      (locale) => !currentLocales.includes(locale),
    );
    // Remove old locales that don't exist in newLocales
    localeConfig.localesToRemove = currentLocales.filter(
    ...
    if (newLocales.includes(defaultLocale)) {
      ...
      localeConfig.localesToAdd = newLocales.filter(
        (locale) =>
          !currentLocales.includes(locale) && locale !== newDefaultLocale,
      );
      // Remove old locales that don't exist in newLocales, except for defaultLocale
      localeConfig.localesToRemove = currentLocales.filter(
        (locale) => !newLocales.includes(locale) && locale !== defaultLocale,
      );
    }
  }
}
