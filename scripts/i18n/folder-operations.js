...
export async function handleFolderOperations(localeConfig, logOptions) {
  ...
  addLocaleFolders(
    ACTIVE_PATHS.srcConfigFolder,
    defaultLocale,
    localesToAdd,
    logOptions,
  );
  addLocaleFolders(
    ACTIVE_PATHS.srcContentFolder,
    defaultLocale,
    localesToAdd,
    logOptions,
  );
 ...
 */
async function updateNavDataReferences(
  defaultLocale,
  newDefaultLocale,
  editOldDefaultToNewDefault,
  logOptions,
) {
  if (!editOldDefaultToNewDefault) return;
 ...
 */
export function determinePagesDirectoryOperations(
  defaultLocale,
  newDefaultLocale,
  editOldDefaultToNewDefault,
  currentLocales,
  ...
  if (
    currentLocales.includes(newDefaultLocale) &&
    newDefaultLocale !== defaultLocale
  ) {
 ...
 */
async function handlePagesDirectory(
  localeConfig,
  currentLocales,
  localesToAdd,
  localesToRemove,
  ...
  if (logOptions.enabled) {
    ...
    log(
      `  - Locales to remove: ${localesToRemove.join(", ") || "none"}`,
      logOptions,
    );
    log(
      `  - Default locale change: ${defaultLocale} → ${newDefaultLocale}`,
      logOptions,
    );
    log(
      `  - Edit old default to new: ${editOldDefaultToNewDefault}`,
      logOptions,
    );
    log(
      `  - Current locales: ${currentLocales.join(", ") || "none"}`,
      logOptions,
    );
  }
  // PLANNING PHASE: Determine which folders to add and remove
  const { foldersToCreate, foldersToDelete } =
    determinePagesDirectoryOperations(
      defaultLocale,
      newDefaultLocale,
      editOldDefaultToNewDefault,
      currentLocales,
  ...
  if (logOptions.enabled) {
    ...
    log(
      `  - Folders to create: ${foldersToCreate.join(", ") || "none"}`,
      logOptions,
    );
    log(
      `  - Folders to delete: ${foldersToDelete.join(", ") || "none"}`,
      logOptions,
    );
  }
  ...
  if (templateFolder && foldersToCreate.length > 0) {
    for (const locale of foldersToCreate) {
      ...
      if (!fssync.existsSync(newLocaleFolder)) {
        ...
        log(
          `Created folder for locale: ${newLocaleFolder} (copied from ${templateFolder})`,
          logOptions,
        );
    ...
    log(
      `Warning: Could not find a template locale folder in ${srcPagesFolder}. No new locale folders created.`,
  ...
  for (const locale of foldersToDelete) {
    ...
    if (fssync.existsSync(localeFolder)) {
      try {
        ...
        console.error(
          `Error deleting folder ${localeFolder}: ${error.message}`,
        );
      }
    }
  }
